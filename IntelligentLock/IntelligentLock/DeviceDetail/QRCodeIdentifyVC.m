...
- (void)initCaptureDevice {
    ...
    if (!input) {
        MPNLog(@"error=%@",[error localizedDescription]);
        return;
    }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        MPNLog(@"metadataObjects = %@", metadataObjects);
        if (self.identifyBlock) {
            self.identifyBlock(obj.stringValue);
        }
...
- (void)scanQRCodeFromPhotosInTheAlbum:(UIImage *)image {
    ...
    MPNLog(@"result:%@",scannedResult);
    if (self.identifyBlock) {
        self.identifyBlock(scannedResult);
    }
    [self.navigationController popViewControllerAnimated:YES];
}
